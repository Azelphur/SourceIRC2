...
{
	decl String:szArg1[IRC_MAXLEN], String:szChannelCmdPrefix[1024], String:szGlobalCmdPrefix[1024];
	// TODO: Prefix support
	...
	else if (!StrEqual(szGlobalCmdPrefix, "") && !strncmp(szArg1, szGlobalCmdPrefix, strlen(szGlobalCmdPrefix))) // Is it using the global command prefix?
		iStartPos = strlen(szGlobalCmdPrefix);
...
public N_IRC_ChannelHasFlag(Handle:plugin, numParams)
{
	...
	// TODO: Check if channel has flag.
	if (StrEqual(szValue, "Disabled"))
		return false;
	return true;
}
...
{
	LogError("IRC_CheckCommandAccess STUB!");
	return true;
}
